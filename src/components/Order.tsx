...
export default function Order({ order }: OrderProps) {
  const paymentStatusMap: Record<orders.PaymentStatus, string> = {
    ...
    [orders.PaymentStatus.UNSPECIFIED]: "Unspecified",
  };
  ...
  const fulfillmentStatusMap: Record<orders.FulfillmentStatus, string> = {
    ...
    [orders.FulfillmentStatus.PARTIALLY_FULFILLED]: "Partially fulfilled",
  };
  ...
  return (
    <div className="w-full space-y-5 border p-5">
      <div className="flex flex-wrap items-center gap-3">
        <span className="font-bold">Order ID: {order.number}</span>
        {order._createdDate && (
  ...
  return (
    <div className="flex flex-wrap gap-3 py-5 last:pb-0">
      <WixImage
        ...
        className="float-none bg-secondary"
      />
      <div className="space-y-0.5">
