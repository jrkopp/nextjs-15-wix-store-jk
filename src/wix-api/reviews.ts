...
export interface CreateProductReviewsValues {
  productId: string;
  title: string;
  body: string;
  rating: number;
...
export async function createProductReviews(
  wixClient: WixClient,
  { productId, title, body, rating, media }: CreateProductReviewsValues,
) {
  const member = await getLoggedInMember(wixClient);

  if (!member) {
    throw Error("You must be logged in to create a review");
  }
